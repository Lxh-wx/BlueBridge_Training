...
- **每道题遇到的逻辑错误和语法错误记录到 错误记录.md**，按日期和题号归档，方便考前回顾易错点。无论错误之前是否犯过都要记录，重复犯错也要写（事后统计频次）。
- **每次训练开始时**：① 创建 `.cpp` 模板框架（如文件已存在则直接打开）② 用 `start chrome` 打开题目网页，优先洛谷（搜题号），洛谷找不到的国赛真题去蓝桥杯官网（https://www.lanqiao.cn/problems/）搜索 ③ 讲思路，让学生自己写代码
...
- **全真模考时**：文件名、注释、模板内容 **不写算法标签和解题提示**，只留题号和题目名。开挂还模考什么？文件名用 `lanqiao-{题号}-{题目名}-{日期}.cpp`，注释只写题意（不含解法）。
